...
class GatewayPayPal extends AbstractGateway
{
    ...
    {
        if (($_ENV['PAYPAL_CLIENT_TEST'] ?? false) == 1) {
            $env = new SandboxEnvironment(($_ENV['PAYPAL_CLIENT_ID'] ?? false), ($_ENV['PAYPAL_CLIENT_SECRET'] ?? false));
        } else {
            $env = new ProductionEnvironment(($_ENV['PAYPAL_CLIENT_ID'] ?? false), ($_ENV['PAYPAL_CLIENT_SECRET'] ?? false));
        }
