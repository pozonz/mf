...
class FormDescriptorBuilder extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        $countryInfo = $request->getSession()->get(UtilsService::COUNTRY_SESSION_KEY);
        if (!$countryInfo) {
            $countryInfo = UtilsService::ip_info($request);
            $countryInfo = $countryInfo ?: [];
            $request->getSession()->set(UtilsService::COUNTRY_SESSION_KEY, $countryInfo);
        }
    ...
    public function getOptionsForField($field): array
    {
        $options = [
...
//                'placeholder' => preg_replace("/[^a-zA-Z0-9\ ]+/", "", $field->label),
    ...
    public function getChoicesForField($field): array
    {
    ...
    public function getValidationForField($field): array
    {
